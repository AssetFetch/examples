...
class Asset:
	...
	def __init__(self, id: str, title: str, description: str, license_spdx: str, author: str, author_uri: str, kind: str) -> None:
		...
		if kind == "material":
			parameters.append(
				templates.HttpParameter(type=templates.HttpParameterType.select,
				id="resolution",
				title="Resolution",
		...
		self.data = datablocks.DataField([
			...
			datablocks.PreviewImageThumbnailBlock(f"{id} Thumbnail", {"256": f"{config.API_URL}/thumbnail/{id}"}),
			datablocks.AuthorsBlock([datablocks.SingularAuthor(author, author_uri, None)])
		])
