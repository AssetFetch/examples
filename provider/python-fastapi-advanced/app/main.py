...
def endpoint_initialization():
	return {
		"meta":
		templates.MetaField(templates.EndpointKind.initialization),
		"id":
		"advanced.examples.assetfetch.org",
		"data":
		datablocks.DataField([
		datablocks.AssetListQueryBlock(uri=f"{config.API_URL}/asset_list",
		method=templates.HttpMethod.GET,
		parameters=[templates.HttpParameter(type=templates.HttpParameterType.text, id="filter", title="Filter", default="")]),
		datablocks.TextBlock("Advanced Example Provider", "This is a more advanced provider for AssetFetch."),
		datablocks.ProviderConfigurationBlock(headers=[datablocks.SingularHeader("access-token", True, True, "Access Token")],
		connection_status_query=templates.FixedQuery(f"{config.API_URL}/status", templates.HttpMethod.GET, {})),
		datablocks.WebReferencesBlock([datablocks.SingularWebReference("AssetFetch Website", "https://assetfetch.org")]),
		...
		datablocks.LicenseBlock("CC0-1.0", None),
		])
	}
...
def endpoint_connection_status(request: Request):
	...
	return {
		"meta":
		templates.MetaField(templates.EndpointKind.connection_status),
		"data":
		datablocks.DataField([
...
def endpoint_asset_list(request: Request, filter: str = ""):
	...
	for asset_yaml_file in asset_yaml_files:
		with open(asset_yaml_file, 'r') as file:
			...
			asset = assets.Asset(id=pathlib.Path(asset_yaml_file).parent.name,
	...
	return {"meta": templates.MetaField(templates.EndpointKind.asset_list), "data": datablocks.DataField([]), "assets": asset_list}
...
def endpoint_implementation_list(asset_id: str, request: Request, response: Response, resolution: int = None, lod: str = None, format: str = None):
	...
	if about_asset['kind'] == "model":
		...
		if len(list(obj_files)) > 0:
			...
			obj_mtl_implementation = implementations.AssetImplementation(f"{implementation_prefix}_obj+mtl", components=[], data=[datablocks.TextBlock("OBJ with MTL file")])
			...
			for obj_path in obj_files:
				...
				obj_component = implementations.AssetImplementationComponent(obj_path.name, [
					datablocks.ObjFormatBlock(datablocks.ObjUpAxis.PLUS_Y),
					datablocks.fetch_download_block_from_path(obj_path, implementation_prefix),
					datablocks.store_block_from_path(obj_path),
					datablocks.HandleNativeBlock()
				])
			...
			for mtl_path in mtl_files:
				...
				mtl_component = implementations.AssetImplementationComponent(mtl_path.name, [
					datablocks.fetch_download_block_from_path(mtl_path, implementation_prefix),
					datablocks.FormatBlock(extension=mtl_path.suffix),
					datablocks.store_block_from_path(mtl_path)
				])
			...
			for jpg_path in jpg_files:
				...
				map_component = implementations.AssetImplementationComponent(jpg_path.name, [
					datablocks.fetch_download_block_from_path(jpg_path, implementation_prefix),
					datablocks.FormatBlock(extension=jpg_path.suffix),
					datablocks.store_block_from_path(jpg_path)
				])
			...
			obj_loose_material_implementation = implementations.AssetImplementation(id=f"{implementation_prefix}_obj+loose_maps",
				components=[],
				data=[datablocks.TextBlock("OBJ with PBR maps")])
			...
			for obj_path in obj_files:
				...
				obj_component = implementations.AssetImplementationComponent(obj_path.name, [
					...
					datablocks.store_block_from_path(obj_path),
					datablocks.HandleNativeBlock(),
					datablocks.LinkLooseMaterialBlock(asset_id)
				])
			...
			for jpg_path in jpg_files:
				...
				if map:
					map_component = implementations.AssetImplementationComponent(jpg_path.name, [
						datablocks.fetch_download_block_from_path(jpg_path, implementation_prefix),
						datablocks.HandleLooseMaterialMapBlock(asset_id, map),
						datablocks.FormatBlock(extension=jpg_path.suffix),
						datablocks.store_block_from_path(jpg_path)
					])
		...
		if len(usd_files) > 0:
			...
			usd_implementation = implementations.AssetImplementation(f"{implementation_prefix}_usd", components=[], data=[datablocks.TextBlock("OpenUSD")])
			...
			for usd_path in usd_files:
				...
				usd_component = implementations.AssetImplementationComponent(usd_path.name, [
					datablocks.fetch_download_block_from_path(usd_path, implementation_prefix),
					datablocks.store_block_from_path(usd_path),
					datablocks.FormatBlock(extension=usd_path.suffix),
					datablocks.HandleNativeBlock()
				])
	...
	if about_asset['kind'] == "material":
		...
		if len(map_files) > 0:
			...
			mat_loose_material_implementation = implementations.AssetImplementation(id=f"{implementation_prefix}_loose",
				components=[],
				data=[datablocks.TextBlock("Set of PBR maps.")])
			...
			for map_path in map_files:
				...
				if map:
					map_component = implementations.AssetImplementationComponent(map_path.name, [
						datablocks.fetch_download_block_from_path(map_path, implementation_prefix),
						datablocks.HandleLooseMaterialMapBlock(asset_id, map),
						datablocks.store_block_from_path(map_path),
						datablocks.FormatBlock(extension=map_path.suffix),
					])
		...
		if len(mtlx_files) > 0:
			mtlx_implementation = implementations.AssetImplementation(f"{implementation_prefix}_mtlx", components=[], data=[datablocks.TextBlock("MTLX Material")])
			...
			for mtlx_path in mtlx_files:
				...
				mtlx_component = implementations.AssetImplementationComponent(mtlx_path.name, [
					datablocks.fetch_download_block_from_path(mtlx_path, implementation_prefix),
					datablocks.FormatBlock(extension=mtlx_path.suffix),
					datablocks.store_block_from_path(mtlx_path),
					datablocks.HandleNativeBlock()
				])
	...
	unlock_queries_block.append(
		datablocks.UnlockQuery(id=implementation_prefix,
		unlocked=(f"{asset_id}/{implementation_prefix}" in user.get_all_purchased_identifiers()),
		price=1,
		query=templates.FixedQuery(f"{config.API_URL}/unlock", templates.HttpMethod.POST, {
		"asset_id": asset_id,
		"implementation_prefix": implementation_prefix
		}),
		query_fallback_uri=None))
	return {"meta": templates.MetaField(templates.EndpointKind.implementation_list), "data": datablocks.DataField([unlock_queries_block]), "implementations": implementation_list}
...
def endpoint_unlock(request: Request, asset_id: str, implementation_prefix: str):
	...
	return {"meta": templates.MetaField(templates.EndpointKind.unlock)}


@app.get("/user/{user}/get_token")
def endpoint_get_token(user: str):
	users.create_if_not_existing(username=user)
	return {"user": user, "new_token": users.DEMO_USERS[user].generate_new_random_token()}
...
def endpoint_set_balance(balance: int, user: str):
	...
	return {"user": user, "new_balance": users.DEMO_USERS[user].balance}
